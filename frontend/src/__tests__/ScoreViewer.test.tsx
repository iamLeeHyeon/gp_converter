...
test('멀티트랙 스코어에서 클릭한 음표의 실제 트랙을 선택한다(항상 트랙 0을 선택하던 버그)', () => {
  const setSelected = vi.fn()
  vi.mocked(useEditorStore).mockReturnValue({
    selected: null, fileId: null, present: null, saveStatus: 'idle',
    setSelected, pushSnapshot: vi.fn(), undo: vi.fn(), redo: vi.fn(), clearHistory: vi.fn(),
  } as any)
  vi.mocked(useEditorStore.getState).mockReturnValue({ present: null } as any)

  render(<ScoreViewer gp5Buffer={new ArrayBuffer(8)} />)
  const calls = vi.mocked(initAlphaTab).mock
  const mockApi = calls.results[calls.results.length - 1].value
  const container = calls.calls[calls.calls.length - 1][0] as HTMLElement

  // 두 번째 트랙(index 1)에 속한 음표 하나를 boundsLookup이 찾아낸 것처럼 모킹한다.
  const beat = { index: 3, voice: { index: 0, bar: { index: 2, staff: { track: { index: 1 } } } } }
  const note = { index: 0, beat }
  mockApi.renderer = {
    boundsLookup: {
      getBeatAtPos: vi.fn().mockReturnValue(beat),
      getNoteAtPos: vi.fn().mockReturnValue(note),
      findBeat: vi.fn().mockReturnValue({ notes: [] }),
    },
  }
  ...
  expect(setSelected).toHaveBeenCalledWith(
    expect.objectContaining({ trackIndex: 1, measureIndex: 2, voiceIndex: 0, beatIndex: 3, noteIndex: 0 }),
  )
})
